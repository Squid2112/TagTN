...
function typeSegLine(tid, newStr, seg, segArray, t) {
	var newStrIdx = 0;

	segArray.unshift("~");
	$.doTimeout(tid, t, function() {
		segArray.reverse();
		segArray.pop();
		segArray.push(newStr.substring(0,newStrIdx++)+"~");
		segArray.reverse();
		seg.html(segArray.toString().replace(/~/g,"<br />"));
		if(newStrIdx > newStr.length) return(false);
		return(true);
	});
}


String.prototype.base64Encode = function(text) {
	var text = new String(this.toString());
...
String.prototype.base64Decode = function() {
	var text = new String(this.toString().replace(/\s/g,''));
