component displayname="VisitorCrudComponent" hint="Visitor CRUD Component" output="false" {
	...
	public any function get(required int visitorId) output="false" {
		var q = new Query();
		...
		q.sql = " 
			SELECT
				...
				vMail.*,
				vIdent.*
			FROM VisitorMaster AS vMast WITH (NOLOCK)
				LEFT OUTER JOIN VisitorEmail AS vMail WITH (NOLOCK) ON vMail.VisitorId = vMast.VisitorId
				LEFT OUTER JOIN VisitorIdentification AS vIdent WITH (NOLOCK) ON vIdent.VisitorId = vMast.VisitorId
			WHERE
				vMast.VisitorId = :visitorId
			";
